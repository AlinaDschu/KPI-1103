...
$("body").on("click", ".btn-delete", function (el) {
...
$("body").on("click", ".btn-edit", function (el) {
  modalEdit.css("display", "flex");
  modalEdit.html(
    `<div class="student-modal">
    <input class="inp-modal name-inp-edit" type="text" value = "${el.target.parentNode.firstChild.nextSibling.innerText.slice(
      5
    )}">
    <input class="inp-modal surname-inp-edit" type="text" value = "${el.target.parentNode.children[1].innerText.slice(
      9
    )}">
    <input class="inp-modal number-inp-edit" type="number" value = "${el.target.parentNode.children[2].innerText.slice(
      7
    )}">
    <input class="inp-modal week-inp-edit" type="number" value = "${el.target.parentNode.children[3].innerText.slice(
      14
    )}">
   <input class="inp-modal month-inp-edit" type="number" value = "${el.target.parentNode.children[4].innerText.slice(
     14
   )}">
  ...
  $("body").one("click", ".btn-save", function () {
    ...
    fetch(`http://localhost:8000/student/${el.target.parentNode.id}`, {
      method: "PATCH",
      headers: { "Content-type": "application/json;charset=utf-8" },
...
$(".search-form input").on("blur", function () {
  fetchProducts(`${API}?_page=${currentPage}&_limit=4`).then((total) => {
    renderPagination(total);
  });
});
...
async function searchRender() {
 ...
 <h4>Имя: ${person.name}</h4>
 <h4>Фамилия: ${person.surname}</h4>
   <h4>Номер: ${person.number}</h4>
  <h4>Неделный KPI: ${person.weekKPI}</h4>
   <h4>Месячный KPI: ${person.monthKPI}</h4>
    ...
    studentList.append(
      `
            <div id="${person.id}" class="student">
                ...
                <h4>Неделный KPI: ${person.weekKPI}</h4>
                <h4>Месячный KPI: ${person.monthKPI}</h4>
                <button class="btn-delete">Удалить студента</button>
...
$("body").on("click", ".page-item", function (el) {
  currentPage = el.target.dataset.page;
